...
// ride-api -> matching-service -> dispatch-service -> ride-api chain.
export function normalTraffic() {
  const res = http.get(`${BASE_URL}/ride-api/request-ride`);
...
// (see services/ride-api/app.py) - asserting that exact code, not just
// "got a response", makes this a real correctness check on the failure path.
export function failureTraffic() {
  const res = http.get(`${BASE_URL}/ride-api/fail`, {
